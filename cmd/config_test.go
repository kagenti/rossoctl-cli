...
func newConfigServer(t *testing.T, body string) *httptest.Server {
	t.Helper()
...
func TestConfigHumanReadable(t *testing.T) {
	...
	out, err := execute(t, "--server", srv.URL+"/api/v1/", "config")
	if err != nil {
...
func TestConfigJSON(t *testing.T) {
	srv := newConfigServer(t, `{"enabled": false}`)

	out, err := execute(t, "--server", srv.URL+"/api/v1/", "config", "--json")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	// Output must be valid JSON with the expected field.
